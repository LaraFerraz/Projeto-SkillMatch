...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($senha !== $confirmar_senha) {
        ...
        $stmt = $conn->prepare("INSERT INTO prestadores (nome, email, telefone, cpf, foto_perfil, tipo_servico, categoria1, categoria2, categoria3, localidade, descricao, horario, senha, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pendente')");
        
        if ($stmt) {
            // Vincula os parâmetros à instrução
            $stmt->bind_param("sssssssssssss", $nome, $email, $telefone, $cpf, $foto_perfil, $servico, $categoria1, $categoria2, $categoria3, $localidade, $descricao, $horario, $senha_hash);
            
            // Executa a instrução
            if ($stmt->execute()) {
                echo "<script>alert('Cadastro realizado com sucesso! Aguarde a aprovação.');</script>";
            } else {
